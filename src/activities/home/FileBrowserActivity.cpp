...
void FileBrowserActivity::loadFiles() {
  ...
  for (auto file = root.openNextFile(); file; file = root.openNextFile()) {
    ...
    if (file.isDirectory()) {
      ...
      } else if (mode == Mode::PickMarkdown) {
        // Markdown picker (vault import): only show .md files.
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Back)) {
    ...
    if (mappedInput.getHeldTime() < GO_HOME_MS) {
      if (basepath != "/") {
        ...
        // Picker mode at root: cancel back to caller instead of going home.
        ActivityResult res;
...
void FileBrowserActivity::render(RenderLock&&) {
  ...
  std::string folderName =
      ...
      : (mode == Mode::PickMarkdown && basepath == "/")
          ? std::string("Select a .md file")
  ...
  if (files.empty()) {
    const char* emptyMsg = (mode == Mode::PickFirmware)   ? tr(STR_NO_BIN_FILES)
                           : (mode == Mode::PickMarkdown) ? "No .md files here"
  ...
  // In picker modes, Confirm on a matching file returns the path to the caller (not "open"); show
  // STR_SELECT instead. Directories in the same picker still descend, so keep STR_OPEN there.
  const bool selectingPickFile = mode != Mode::Books && !files.empty() && files[selectorIndex].back() != '/';
